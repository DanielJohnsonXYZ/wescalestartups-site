...
1. **Consolidate the fractional-CMO cluster.** Seven pages compete for one head term: 3,311 impressions, **zero clicks**, 90 days. `/services/fractional-cmo` (2,035 imp, pos 49.0) and `/fractional-cmo-vs-agency` (1,062 imp, pos 45.1) carry 94% of it. Full per-page proposal with 301 map: `wss-consolidation-proposal.md` (delivered to Daniel 21 Aug, awaiting approval). **The internal-linking hierarchy fix inside it is the highest-value, lowest-risk item on this list** — no URLs change.
2. **Off-site authority (§3).** Wikidata items for WSS and Daniel; fact consistency across LinkedIn / Crunchbase / GrowthMentor / MentorCruise / Companies House; earned mentions over links. This is where position actually moves. Requires Daniel — the bot cannot create Wikidata items or edit third-party profiles.
...
8. **`cal.wescalestartups.com/auth/login` is indexed.** Lives on Steve, not this repo — flag to Daniel, don't ship.
9. **Two live 404s on the `www.` host:** `/partner-view/university-of-cambridge/` and `/services-mobile-marketing/`. Note a 301 exists for the *misspelled* `university-of-camebridge` but not the correct spelling.
