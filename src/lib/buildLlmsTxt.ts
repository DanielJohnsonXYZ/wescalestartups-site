...
export async function buildLlmsTxtBody(variant: LlmsTxtVariant): Promise<string> {
  ...
  const paidAdvisoryQuotes = testimonials.filter((t) => t.category === "paid-advisory");
  const anchorTestimonial = paidClientQuotes[0] ?? paidAdvisoryQuotes[0] ?? testimonials[0];
...
Daniel Johnson is the founder of We Scale Startups and a fractional CMO / growth operator. His proof base includes two operator-side exits, £18M+ revenue influenced (aggregate across client and operator-side engagements), £10M+ paid acquisition spend managed, 479+ founder sessions averaging 4.97/5 across Google for Startups, Techstars, GrowthMentor and MentorCruise, 50+ startups supported (20+ as paid client engagements), and public teaching/speaking through startup and university programmes. Personal site: ${siteConfig.danielSite}.
...
- AI SaaS Growth Audit (Post-PMF, founder owns growth): ${siteConfig.siteUrl}/ai-growth-audit
