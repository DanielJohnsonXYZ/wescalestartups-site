...
| `sticky_book_cta_shown` | Sticky bar becomes visible | `sticky_book_cta_shown` | No |
...
3. **Triggers** → Custom Event:
   ...
   - `ce_sticky_book_cta_shown` → Event name `sticky_book_cta_shown`
4. **Tags** → GA4 Event tags (Config tag already on container):
   ...
   - Tag `GA4 - sticky_book_cta_shown` → Event name `sticky_book_cta_shown` → params `page_path`, `ai_referral` → trigger `ce_sticky_book_cta_shown`
